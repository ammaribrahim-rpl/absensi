...
if (isset($_POST['simpan_password'])) {
    ...
    if (empty($pass_lama) || empty($pass_baru) || empty($konfirmasi)) {
        ...
        // Cek password lama di tb_daftar
        $stmt_cek = mysqli_prepare($koneksi, "SELECT password FROM tb_daftar WHERE username = ? LIMIT 1");
        ...
        if ($data_user && password_verify($pass_lama, $data_user['password'])) {
            ...
            $stmt_upd  = mysqli_prepare($koneksi, "UPDATE tb_daftar SET password = ? WHERE username = ?");
            mysqli_stmt_bind_param($stmt_upd, "ss", $pass_hash, $username);
            if (mysqli_stmt_execute($stmt_upd)) {
                $pesan_sukses = 'Password berhasil diubah! Gunakan password baru untuk login berikutnya.';
            } else {
...
<head>
    ...
    <title>Ganti Password — Absensi Admin</title>
                ...
                <div class="section__content section__content--p30">
                    <div class="container-fluid">
                        <div class="header-wrap">
                            <div>
                                <h4 class="font-weight-bold mb-0 text-dark">Ganti Password Admin</h4>
                                <small class="text-muted">Keamanan Akun Administrator</small>
                            </div>
                    ...
                    <div class="container-fluid">
                        ...
                        <div class="card card-password p-4">
                            <div class="d-flex align-items-center mb-3">
                                ...
                                <div>
                                    <h5 class="font-weight-bold text-dark mb-0">Ubah Password Akun Admin</h5>
                                    <p class="text-muted small mb-0">Username: <strong>@<?= htmlspecialchars($username) ?></strong></p>
                                </div>
                            </div>
                            <hr class="mt-2 mb-4">
                            ...
                            <form action="" method="POST">
                                <div class="form-group mb-3">
                                    <label class="font-weight-bold text-muted small">PASSWORD LAMA</label>
                                    <input type="password" name="pass_lama" class="form-control" placeholder="Masukkan password lama Anda" required autocomplete="off">
                                </div>
                                <div class="form-group mb-3">
                                ...
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <button type="submit" name="simpan_password" class="btn btn-primary font-weight-bold">
